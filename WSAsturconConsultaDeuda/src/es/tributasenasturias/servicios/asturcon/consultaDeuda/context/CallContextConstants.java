/**
 * 
 */
package es.tributasenasturias.servicios.asturcon.consultaDeuda.context;

/** Constantes de contexto.
 * @author crubencvs
 *
 */
public final class CallContextConstants {
	private CallContextConstants(){};
	public static final String PREFERENCIAS="PREFERENCIAS";
	public static final String LOG_APLICACION="LOG_APLICACION";
	public static final String ID_SESION="ID_SESION";
	public static final String TEXTO_XML="TEXTO_XML";
}
