...
public class CargaContexto implements SOAPHandler<SOAPMessageContext> {
	...
	private void destruirContextoSesion(SOAPMessageContext context)
	{
		...
		context.remove(Constantes.ERROR_SEGURIDAD);
	}
	@Override
	...
	public boolean handleMessage(SOAPMessageContext context) {
		...
		try
		{
			...
			if (!salida.booleanValue())
			{
				...
				ILog log= LoggerFactory.getLogger(pref.getModoLog(),pref.getFicheroLogAplicacion(),idLlamada);
				context.put (Constantes.LOG, log);
				context.setScope(Constantes.LOG,MessageContext.Scope.APPLICATION);
				//Inicializamos el error de validación de seguridad, aunque no le daremos valor hasta más adelante
				context.put (Constantes.ERROR_SEGURIDAD, Boolean.FALSE);
				context.setScope(Constantes.ERROR_SEGURIDAD,MessageContext.Scope.APPLICATION);
			}
			else
			{
				destruirContextoSesion(context);
		...
		catch (PreferenciasException ex)
		{
			...
			System.err.println ("Servicio Adviser de Notific@:: error en preferencias ::"+ex.getMessage());
			ex.printStackTrace();
