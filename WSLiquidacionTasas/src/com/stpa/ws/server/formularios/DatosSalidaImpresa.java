...
import com.stpa.ws.server.util.InorInfoOrga;
import com.stpa.ws.server.util.ListadoUtils;
import com.stpa.ws.server.util.Oficina;
import com.stpa.ws.server.util.Organismo;
...
public class DatosSalidaImpresa {
	 ...
	 * @return boolean
	 */
	protected static boolean validString(String cadena) {
	 ...
	 * @return String
	 */
	protected static String traduc(String dato, String idioma) {		
		return dato;
	}
	...
	public static double Val(String input) {
		...
		try {
			...
			//e.printStackTrace();
			com.stpa.ws.server.util.Logger.errors("Error:"+e.getMessage(),e,com.stpa.ws.server.util.Logger.LOGTYPE.APPLOG);
		}
	 ...
	 */
	protected static String campoFecha(Map rs, String nombre) {
	...
	public static void dependenciasOrganismo(Document doc) {
		...
		for (int j = 0; j < campos.length; j++) {
			...
			if (c.getTextContent().equals("icono")) {				
				String literal = LiteralesUtils.obtenerLiteralFromInorInfoOrga(organismo, "IL");
	...
	public static String rellenaTexto(String idText, String idOrga) {
		...
		try
		{
			organismo = ListadoUtils.obtenerOrganismo(idOrga);
		}
		catch (Exception e)
		{
			organismo=null;
					...
					if (estado == 1)
						...
						if (codTiio.equals(""))
							...
							if (codTiioMin.equals("nombre_orga")){
								...
								InorInfoOrga inorInfo=null;
								try{
									inorInfo = ListadoUtils.obtenerInorInfoOrga(idOrga, codTiio);
								} catch (Exception e)
								{
									inorInfo=null;
								}
	...
	public static String rellenaOficinas(String idOrga, int orden) {
		...
		*/
		...
		try
		{
			oficinas = ListadoUtils.obtenerOficinasBySaleRecibo("S");
		}
		catch (Exception e)
		{
			oficinas=null;
	...
	public static String rellenaOficinasLiquidadoras(String idOrga, int orden) {
		...
		List<Oficina> oficinasLiq=null;
		try
		{
			oficinasLiq = ListadoUtils.obtenerOficinasLiquidadoras();
		}
		catch (Exception e)
		{
			oficinasLiq=null;
	...
	public String pdf_open_image_file(Node node) {
		...
		try {
			...
			System.err.println("ERROR. No se puede cargar la imagen " + fichero);
			e.printStackTrace(System.out);
		}
		if (pdfimage == null) {
			System.err.println("ERROR. No se puede abrir la imagen " + fichero);
			return "error";
		} else {
	...
	public String pdf_linea(Node nodo) {
		...
		if (error != 0) {
			com.stpa.ws.server.util.Logger.errors(errormsg,null,com.stpa.ws.server.util.Logger.LOGTYPE.APPLOG);
