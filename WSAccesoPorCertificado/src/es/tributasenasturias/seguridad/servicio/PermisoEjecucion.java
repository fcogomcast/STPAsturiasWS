...
public class PermisoEjecucion{
	...
	private stpa.services.LanzaPLService instanciarServicio()
	{
		...
		lW=new stpa.services.LanzaPLService ();
		return lW;
	}
	/**
	...
	public DatosPermisosServicio permisoServicio(String aliasServicio,String cifNif) throws SeguridadException
	{
		...
		try
		{
			...
			RespuestaLanzador res = new RespuestaLanzador(resultadoEjecutarPL);
			DatosPermisosServicio resultado = SeguridadFactory.newDatosPermisosServicio();
			if (!res.esErronea())
			{
				String autorizacion =res.getValue("CADE_CADENA", 1, "STRING_CADE"); 
				if (autorizacion==null)
				{
					throw new SeguridadException ("Estructura CADE_CADENA no contiene el código de autorización sobre el servicio.");
				}
				...
				else
				{
					...
					//Recuperamos la lista de datos de usuarios
					int filasCanu = res.getNumFilasEstructura("CANU_CADENAS_NUMEROS");
					EstructuraLanzador canu = res.getEstructura("CANU_CADENAS_NUMEROS");
					for (int i=1;i<=filasCanu;i++)
					{
						DatosUsuarioTributas usu = new DatosUsuarioTributas();
						usu.setUsuarioTributas(canu.getFila(i).getCampo("STRING1_CANU"));
						usu.setPasswordTributas(canu.getFila(i).getCampo("STRING2_CANU"));
						usu.setIdFiscal(canu.getFila(i).getCampo("STRING3_CANU"));
						usu.setSuborganismo(canu.getFila(i).getCampo("STRING4_CANU"));
						usu.setIdSuborganismo(canu.getFila(i).getCampo("NUME1_CANU"));
						resultado.getListaUsuarios().add(usu);
					}
				}
			}
			else
