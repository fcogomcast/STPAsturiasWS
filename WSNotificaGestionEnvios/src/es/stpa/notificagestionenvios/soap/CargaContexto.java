...
public class CargaContexto implements SOAPHandler<SOAPMessageContext> {
	...
	private void destruirContextoSesion(SOAPMessageContext context)
	{
		...
		context.remove(Constantes.IDLLAMADA);
	...
	public boolean handleMessage(SOAPMessageContext context) {
		...
		try
		{
			...
			if (!salida.booleanValue())
			{
				...
				context.put(Constantes.IDLLAMADA, idLlamada);
				context.setScope(Constantes.IDLLAMADA, MessageContext.Scope.APPLICATION);
		...
		catch (PreferenciasException ex)
		{
			...
			System.err.println ("Gestión de envíos de Notifica:: error en preferencias ::"+ex.getMessage());
			ex.printStackTrace();
			SOAPUtils.generateSOAPErrMessage(context.getMessage(),"Error en proceso de mensaje SOAP.","0001","Error al crear el entorno de la llamada:"+ex.getMessage(), true);
		} 
		return true;
	}
