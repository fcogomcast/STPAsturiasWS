...
public class ConversorRespuestaCaixa {
	...
	/**
	 * Devuelve el mensaje equivalente al de entrada de LaCaixa. Si no hay uno particular, devuelve el error genérico.
	 * @param errorLaCaixa
	 * @return
	 */
	...
	public String getResultadoCaixa (String resultadoCaixa)
	{
		...
		if (resultadoCaixa!=null && resultadoCaixa.length()>=Constantes.getLongColumnaRespuesta())
		{
			...
			if (idxEspacio!=-1)
			{
				res=resultadoCaixa.substring(0,idxEspacio); // Código de resultado en la Caixa.
			...
			else
			{
				...
				{
					res=resultadoCaixa.substring(0,Constantes.getLongColumnaRespuesta()); // Código de resultado en la Caixa.
				}
				else
				{
					res=resultadoCaixa;
