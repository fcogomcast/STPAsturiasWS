...
public class ObtenerListadosWS extends GeneralWS {
	...
	public static List<Map<String,String>> obtenerListado(String procedimiento, List<CampoWS> camposEntrada, 
		...
		try {			
			//es.tributasenasturias.documentopagoutils.Logger.info("procedimiento"+procedimiento,es.tributasenasturias.documentopagoutils.Logger.LOGTYPE.APPLOG);
			String peticion = generarPeticion(procedimiento, camposEntrada);		
			String xmlOut = llamadaWebService(peticion);
			es.tributasenasturias.documentopagoutils.Logger.info("peticion"+peticion,es.tributasenasturias.documentopagoutils.Logger.LOGTYPE.APPLOG);
			listado = respuestaWSObtenerListado(xmlOut, camposRecuperar, campoError);
			
		} catch (Exception e) {
			//es.tributasenasturias.documentopagoutils.Logger.info("error en obtenerlistado"+e.getMessage(),es.tributasenasturias.documentopagoutils.Logger.LOGTYPE.APPLOG);
