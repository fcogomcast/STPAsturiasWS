	...
	public static ByteArrayOutputStream descomprimirPDF(String pdfComprimido) throws Exception
	{
		...
		if(pdfComprimido != null){
			...
			try { 
				...
				Logger.error(msgLog+"Error al descomprimir PDF|"+e.getMessage(),es.tributasenasturias.documentopagoutils.Logger.LOGTYPE.APPLOG);				
	...
	public static String comprimirPDF(ByteArrayOutputStream pdf) throws Exception
	{
		...
		if(pdf != null){
			try{
				...
				Logger.info(msgLog,es.tributasenasturias.documentopagoutils.Logger.LOGTYPE.APPLOG);
