...
public class GenericAppLogger implements ILoggerAplicacion {
	...
	public enum LEVEL {NONE,INFO, DEBUG,WARNING, SEVERE,ALL}
	...
	private boolean esImprimible(LEVEL nivel)
	{
		...
		try
		{
			...
			{
				if (nivel.equals(LEVEL.DEBUG)|| nivel.equals(LEVEL.WARNING)|| nivel.equals(LEVEL.SEVERE))
				{res=true;}
				else {res=false;}
			}
	...
	public synchronized void doLog(String message, LEVEL level)
	{
        ...
        {
            Date today = new Date();
            String completeMsg = _nombre+" :: "+"::" + GestorIdLlamada.getIdLlamada() +"::"+ today + " :: " + level + " :: " + message;
        ...
        {
            System.out.println("Error escribiendo log " + _nombre + "::"+ GestorIdLlamada.getIdLlamada()+ " ::'"+message+"' -> "+e.getMessage());
            e.printStackTrace();
        }
        finally
        {
        	if (pw!=null)
        	{
            	...
            	{
            		System.out.println ("Error cerrando flujo de impresión para el log "+ _nombre + "::"+ GestorIdLlamada.getIdLlamada() + ":: " + e.getMessage());
            		e.printStackTrace();
            	}
        	}
            if(fichero != null)
            {
                ...
                {
                    System.out.println("Error cerrando fichero de log " + _nombre + "::"+ GestorIdLlamada.getIdLlamada()+ "-> "+e.getMessage());
                    e.printStackTrace();
                }
            }
        }
