...
public class SoapClientHandler implements SOAPHandler<SOAPMessageContext> {
	...
	private void log(SOAPMessageContext context)
	{
		...
		catch (javax.xml.soap.SOAPException ex)
		{
			...
			SOAPUtils.generateSOAPErrMessage(context.getMessage(), "Error en proceso de comunicación Traslado Máquina <--> Servicio Remoto."	, "0002", "SOAP Handler", true);
		}
		catch (java.io.IOException ex)
		{
			...
			SOAPUtils.generateSOAPErrMessage(context.getMessage(), "Error en proceso de comunicación Servicio Códigos de  barras <--> Servicio Remoto."	, "0002", "SOAP Handler", true);
		}
		catch (PreferenciasException ex)
		{
			//En este punto es seguro que no tenemos log. Grabamos en la consola de servidor.
			System.err.println ("Servicio de códigos de barras: Error de preferencias en manejador SOAP ("+SoapClientHandler.class.getName()+":"+ex.getMessage());
			SOAPUtils.generateSOAPErrMessage(context.getMessage(), "Error en proceso de comunicación Servicio Códigos de barras <--> Servicio Remoto."	, "0002", "SOAP Handler", true);
		}
	}
	@Override
