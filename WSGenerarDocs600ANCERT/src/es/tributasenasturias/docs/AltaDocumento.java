...
import es.tributasenasturias.GenerarDocs600ANCERTutils.PdfComprimidoUtils;
import es.tributasenasturias.GenerarDocs600ANCERTutils.Preferencias;
import es.tributasenasturias.docs.Base64; //import es.tributasenasturias.documentos.util.XMLUtils; //import es.tributasenasturias.GenerarDocs600ANCERTutils.ConversorParametrosLanzador;
...
public class AltaDocumento {
	...
	public String setDocumento(String p_tipo, String p_nombre,
		...
		try {
			...
			if (pref.getDebug().equals("1")) {
				Logger.info("**** LLAMADA PROCEDIMIENTO ALTA DOCUMENTO ****");
				Logger.info("Procedimiento:" + pref.getPAAltaDocumento());
				Logger.info("Entorno:" + pref.getEntorno());
				Logger.info("Codifica:" + cpl.Codifica());
				Logger.info("EndPointLanzador:" + pref.getEndpointLanzador());
