...
public class AutenticacionPAHelper {
	...
	private String construyePeticion(String certificado)
	{
		...
		{			
			Logger.error("Error de XML:" + ex.getMessage());
			xml=null;
		}
		catch (Exception ex)
		{
	...
	public String login (String certificado) throws ValidacionException
	{
		...
		{									 					
			...
			if (pref.getDebug().equals("1")) {
				Logger.info("LOGIN::Certificado:" +certificado);
				Logger.info("endopint autentica:" +dirServicio);
				Logger.info("Peticion construida para certificado:" +peti);			 
			}
		...
		catch (MalformedURLException ex)
		{
			...
			Logger.error("Error al verificar la autenticación.Exceptión de servicio." + ex.getMessage());
			throw new ValidacionException ("Error al verificar la autenticación.");
		}		
