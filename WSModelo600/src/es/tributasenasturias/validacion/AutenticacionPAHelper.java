...
public class AutenticacionPAHelper implements ILoggable {
	...
	private String getIdentificadorRespuesta (String xml) {
		...
		try{
			...
			nodos = XMLDOMUtils.getAllNodesXPath(doc,"//item[contains(name,'CIF')]");
			...
			if (nodos.getLength()!=0){
				...
				nodos = XMLDOMUtils.getAllNodesXPath(doc,"//item[contains(name,'NIF')]");
				if (nodos.getLength()!=0){
	...
	private String construyePeticion(String certificado){
		...
		try{
			...
			if (nombreFichero!=null && !nombreFichero.equals("")){
				...
				nodos = XMLDOMUtils.getAllNodesXPath(doc,"//item[contains(name,'IP')]"); 
