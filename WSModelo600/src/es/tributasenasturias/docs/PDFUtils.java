		...
		//if(xmlTasasLiq!=null){
			try {				
				...
				if (bufferArray[0].toByteArray().length == 0){
					throw new PresentacionException("NO SE HA COMPILADO EL PDFBEAN DocComparencia");
				}
			} catch (RemoteException e) {
				throw new PresentacionException("Error generando PDF DocComparencia " + e,e);
			}
		//}
...
//		escribirPdf(buffer,"c:/AmbasTasas.pdf");
	...
	public static String getContents(String aFile, String encoding) throws PresentacionException {
		try {
			...
			throw new PresentacionException("Error obteniendo XSL "+ex.getMessage(),ex);
		}
	}

	public static String getContents(URL aFile, String encoding) throws PresentacionException{
		...
		try {
			...
			throw new PresentacionException("Error obteniendo XSL "+e.getMessage(),e);
		}
	}
